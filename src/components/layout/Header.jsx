...
function Header({ darkMode, setDarkMode, handleToggle }) {
  ...
  return (
    <header className="flex items-center justify-between w-full h-20">
      {/* Logo */}
      <img src={logoNews} alt="logo" className="dark:invert" />
      ...
      {/* Sidebar en mobile */}
      {isMenuOpen && (
        <aside className="fixed top-0 right-0 w-64 h-full bg-Off-white dark:bg-Very-dark-blue z-50 pt-20 flex flex-col gap-6">
          <NavBar
            handleFilterClick={handleFilterClick}
            darkMode={darkMode}
            setDarkMode={setDarkMode}
